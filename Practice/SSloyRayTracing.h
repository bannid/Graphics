...
#pragma omp parallel for
		for (size_t j = 0; j < height; j++) { // actual rendering loop
			for (size_t i = 0; i < width; i++) {
				...
				finalColor.red = std::max(0, std::min(255, finalColor.red));
				finalColor.green = std::max(0, std::min(255, finalColor.green));
				finalColor.blue = std::max(0, std::min(255, finalColor.blue));
