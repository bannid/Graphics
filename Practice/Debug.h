...
#define TIMED Timer t(__LINE__,__FILE__,__FUNCTION__)
//Currently can be used inside the classes only where 
//we have timingData map defined.
#define TIMED_DATA Timer t(__LINE__,__FILE__,__FUNCTION__,&this->timingData)
struct Timer {
	...
	~Timer() {
		...
		if (this->timingData != nullptr) {
			std::string s(this->functionName);
